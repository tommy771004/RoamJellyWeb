...
const targetIndex = code.indexOf(`aria-label={isLoggedIn ? '個人檔案' : '登入'}`);
if (targetIndex !== -1) {
  const startIndex = code.lastIndexOf('<button', targetIndex);
  let endIndex = code.indexOf('</button>', targetIndex);
  if (endIndex !== -1) {
    endIndex += '</button>'.length;
  ...
  console.log("Could not find aria-label");
}
