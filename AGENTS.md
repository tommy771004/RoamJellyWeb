...
- src/store/useTripFactsStore.ts: 行程基本資訊快取 (trip facts / preferences)
...
6. 工作樹根目錄有未追蹤的 `.superpowers/brainstorm/` 資料夾，內含 visual companion 產生的
   設計腦暴快照（session `visual-1778245069`）。這不是專案程式碼，不要 commit 進 git；
   若要清除可直接刪除整個 `.superpowers/` 目錄，不影響任何功能。
...
- **[A] Navigate + Free Maps deep-link**
  ItineraryTab 節點點擊後帶 lat/lng 開啟 Google Maps / Apple Maps；
  需補 workflowApi 映射與 App.tsx URL scheme 判斷。

- **[B] 環境收尾 + typecheck/build 驗證**
  清理 node_modules → `npm ci` → `npx tsc --noEmit` → `npm run build`；
  確認 migrations/0003_misty_jellyfish.sql 與 schema.ts 同步後再合入主線。

- **[C] Spot-level Regenerate（主線優先）**
  在 AI 規劃流程中支援單一節點重新產生，而非整份行程重跑；
  需更新 aiItineraryService.ts + POST /api/itinerary/regenerate-spot。
